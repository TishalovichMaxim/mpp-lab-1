...
public class TestsGenerator
{
    ...
    public Task Generate(IList<string> files, string resDir, int maxInFiles, int maxProcFiles, int maxOutFiles)
    {
        ...
        TransformBlock<string, string> readingFilesBlock = new(
        ...
        ActionBlock<TestClassInfo> outputBlock = new(
            async info => await File.WriteAllTextAsync(Path.Combine(resDir, info.ClassName + ".cs"), info.Content),
            outputBlockOptions
        );
    ...
    public MethodDeclarationSyntax CreateDefaultTestMethod(MethodDeclarationInfo methodInfo)
    {
        return
                MethodDeclaration(
                    PredefinedType(
